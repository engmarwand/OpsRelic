...
export default function Dashboard() {
  ...
  const topInsight = useMemo(() => {
    ...
    if (sortedPlatforms.length >= 2) {
      ...
      const diffPct = Math.ceil((top[1] - second[1]) / second[1] * 100);
    ...
    return (
      ...
      >
        <motion.div 
          ...
          className="w-24 h-24 rounded-[40px] bg-blue-600/5 border border-blue-500/10 flex items-center justify-center mb-8 relative group"
        >
        ...
        <h2 className="text-3xl font-black text-white italic uppercase tracking-tighter mb-4">Awaiting Signal</h2>
        <p className="text-[#555] mb-10 max-w-md text-[11px] font-bold uppercase tracking-widest leading-relaxed">
          Your agency oversight dashboard is connected and primed. It will automatically generate insights and KPIs once performance data is ingested from your campaigns.
        </p>
        <div className="flex items-center gap-4">
          <a 
            href="#pipeline" 
            className="text-white px-8 py-4 rounded-2xl text-[10px] font-black uppercase tracking-[0.3em] transition-all bg-blue-600 shadow-xl shadow-blue-600/20 active:scale-95"
          >
            Go to Pipeline
          </a>
          <a 
            href="#upload" 
            className="text-white px-8 py-4 rounded-2xl text-[10px] font-black uppercase tracking-[0.3em] transition-all bg-white/5 border border-white/5 hover:bg-white/10 active:scale-95"
          >
            Import Data
          </a>
        </div>
  ...
  return (
    <div className={`space-y-8 ${layout === 'Condensed' ? 'text-sm' : ''}`}>
      ...
      {/* Top Banner Insight */}
      <div className={`relative overflow-hidden rounded-[48px] border border-white/5 bg-[#0A0A0A] p-10 group/insight ${!hasFeature('aiInsights') ? 'opacity-90' : ''}`} style={{ boxShadow: `0 40px 80px -20px ${primaryColor}15` }}>
      ...
      <div className={`grid grid-cols-1 lg:grid-cols-5 ${getGap()}`}>
        <div className={`lg:col-span-3 bg-[#0A0A0A] border border-white/5 p-8 lg:p-10 rounded-[48px] shadow-2xl relative overflow-hidden group/chart`}>
        ...
        <div className={`lg:col-span-2 bg-[#0A0A0A] border border-white/5 p-8 lg:p-10 rounded-[48px] shadow-2xl relative flex flex-col`}>
          ...
          <div className="flex-1 flex items-center justify-center relative min-h-[400px]">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
              <span className="text-3xl font-display font-black tracking-tighter text-white/20 tabular-nums leading-none">{platformData.length > 0 ? Math.ceil((platformData[0].value / totalViews) * 100) : '0'}%</span>
      ...
      <div className={`grid grid-cols-1 lg:grid-cols-2 ${getGap()}`}>
        <div className="bg-[#0A0A0A] border border-white/5 rounded-[48px] shadow-2xl overflow-hidden flex flex-col">
